...
PACKAGES="$(jq -r '.name // ""' <<< "$IPKG") $PACKAGES"
...
for packageName in $PACKAGES prelude; do
    for docs in                                                       \
        ...
        "$PACKPATH/install/$COMMIT/$packageName"/*/*/*/docs           \
        "$PWD/$BUILDDIR/docs"                                         ; do

        cp -r "$docs" "$OUTDIR/data/$packageName";
    done
done

(
    cd $BUILDERDIR;
    poetry run ./mkindex.py "$OUTDIR/data"
    poetry run ./mkhome.py "$OUTDIR/data"
    cp -r app.js index.html style.css $OUTDIR
)

(
    cd $OUTDIR;
    python3 -m http.server 8001 || true;
)
